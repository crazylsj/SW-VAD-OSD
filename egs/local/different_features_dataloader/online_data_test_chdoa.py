...
class OnlineFeats_test(Dataset):
    ...
    def __init__(self, ami_audio_root, label_root, configs, segment=500, probs=None, synth=None):
        ...
        in_channels=8
                 # radius=None,
        radius=0.1
        theta_array = 2 * numpy.pi / in_channels * torch.zeros(in_channels,1, 1)
        # theta_array = 2 * numpy.pi / in_channels * torch.zeros(1, in_channels, 1, 1)
        ch_order_set = torch.arange(-ch_order, ch_order + 1, 1, dtype=float)
        
        self.ch_order = ch_order
        self.in_channels = in_channels
        # stft_kwargs["in_channels"] = self.in_channels
       
        self.theta_array = torch.nn.parameter.Parameter(theta_array, requires_grad=False)
        # self.stft = MultichannelSTFT(**self.stft_kwargs)
        self.ch_order_set = torch.nn.parameter.Parameter(ch_order_set, requires_grad=False)
        # print(f'self.ch_order_set:{self.ch_order_set}')

        # by default, no need of the wavenumber
        self.k = None
        self.radius = radius
        if self.radius is not None:
            fs = 16000
            c = 342.0
            freqs = torch.linspace(0.0, fs / 2, 400 // 2 + 1)
            # wavenumber
            self.k = 2 * numpy.pi * freqs / c
    ...
    def __getitem__(self, item):
        ...
        x = self.stft(concatenated_audio).transpose(0, 1)
        # print(f'x_stft:{x_stft.shape}')
        # print(f'x_stft:{x_stft.shape}') # [64,8,257,497] 
        circular_harm = torch.zeros((len(self.ch_order_set), x.shape[1], x.shape[2]), dtype=torch.float,
                                    device=x.device)

        for ii in range(len(self.ch_order_set)):
            if self.k is None:
                circular_harm[ii,:,:] = torch.sum(
                    x * torch.exp(-1j * self.ch_order_set[ii] * self.theta_array) / jn_kr, dim=0)
            else:
                v = self.ch_order_set[ii]
                z = self.k * self.radius
                v_cpu = v.detach().cpu().numpy()
                z_cpu = z.detach().cpu().numpy()
                # print(v.device)
                # print(z.device)
                # jn_kr = special.jv(v, z).repeat(x.shape[-1], 1).permute(1, 0)
                jn_kr = torch.tensor(special.jv(v_cpu, z_cpu))
        
                jn_kr = jn_kr.to(x.device).repeat(x.shape[-1], 1).permute(1, 0)
                

                # jn_kr = special.jv(v, z).to(x.device).repeat(x.shape[-1], 1).permute(1, 0)
                jn_kr[jn_kr == 0] = 1e-3

                circular_harm[ii,:,:] = torch.sum(
                    x * torch.exp(-1j * self.ch_order_set[ii] * self.theta_array) / jn_kr, dim=0)

        # return torch.sin(torch.flatten(circular_harm/self.in_channels,start_dim=1,end_dim=2).angle())
        sin_sum_ch = torch.sin(torch.sum(circular_harm / self.in_channels, dim=0).angle())
        cos_sum_ch = torch.cos(torch.sum(circular_harm / self.in_channels, dim=0).angle())
        # print(f'sin_sum_ch:{sin_sum_ch.shape}')
        array_fea =  torch.cat([sin_sum_ch, cos_sum_ch], dim=0)
